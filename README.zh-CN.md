...
一个用于电商新品上架准备的 Agent Skill，覆盖淘宝、拼多多、抖音、亚马逊、1688、快手等渠道，把产品信息、市场证据、合规检查、创意方向、商品文案、客服话术和上架动作整理成一份可复核的执行包。
...
| 问题 | 回答 |
|---|---|
| 适合谁 | 正在准备新品上架的运营、创始人、设计、客服团队和 Agent 用户。 |
| 会输出什么 | 定位、证据标签、商品文案、图视频方向、FAQ、客服话术和执行计划。 |
| 重点保护什么 | 平台合规、结论可追溯、预算纪律，以及多角色之间的交付清晰度。 |
| 最适合的第一次使用 | 给 Skill 一份商品 brief，让支持 Skill 的 Agent 跑完整上架流程。 |
...
- 配套调研和抓取工具：`multi-search-engine`、`anysearch`、Firecrawl、`agent-reach`、Tavily。
- 中文文案去 AI 味：`humanizer-zh`，以及本 Skill 内置的反模板写作规则。
- 动画演示链路：HyperFrames、GPT Image 2 生成场景、MiniMax CLI 背景音乐。
...
当前公开包已经包含 Skill 流程、配套 Skill 策略、合规参考、示例、贡献指南、安全审计和中英文 README。仓库保持 provider-neutral，不内置私有 API key，也不绑定私有生图或视频路线。
