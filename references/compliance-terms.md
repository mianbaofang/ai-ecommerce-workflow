...
此文件是 `ai-ecommerce-workflow` 的内置文案审核参考。风险词命中只负责定位，不等于自动判定违法；最终结合原句、商品品类、证据、目标地区和当前平台规则判断。平台长度、字段和禁限售规则易变化，正式上架前以当前后台为准。
...
本文件不是法律意见或平台审核结果。详细审核流程见仓库根目录 `references/listing-compliance-review.md`。
